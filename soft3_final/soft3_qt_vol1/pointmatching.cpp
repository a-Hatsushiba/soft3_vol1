...
void pointMatching::run(void)
{
  ...
  if(num == 2){
    ...
    cv::imwrite(g_img_path1, image1);
    cv::imwrite(g_img_path2, image2);
    if(DEBUG){
        cv::imshow("image1_cro", image1);
        cv::imshow("image2_cro", image2);
//        cv::waitKey(10);
    }
  ...
  //特徴点マッチング
  if(DEBUG) std::cout << "特徴点マッチング始まるよ" << std::endl;
  ...
  if(end_match == 0){ //特徴点マッチングで類似度高い
      ...
      absdiff1(image1, image2); //差分検出方法①(ファイル向け) -> 長方形補正後ならカメラでも使える
...
void pointMatching::cropping(const cv::Mat &src, cv::Mat &dst){
  ...
  cv::dilate(ret, ret, cv::Mat(), cv::Point(-1,-1), 2);//ココらへんを変える(黒収縮)
  cv::erode(ret, ret, cv::Mat(), cv::Point(-1,-1), 2);//ココらへんを変える(黒膨張)
  ...
  trans = cv::Mat::zeros(cv::Size(dst_width, dst_height), CV_32F);
  cv::Mat H = cv::getPerspectiveTransform(src_vertex, dst_vertex);
  std::cout << "check" << std::endl;
  cv::warpPerspective(src, trans, H, dst.size());
  dst = trans.clone();
  if(DEBUG) cv::imshow("dst透視", dst);
}
...
int pointMatching::feature_matching(const cv::Mat &src1, const cv::Mat &src2, cv::Mat &dst1, cv::Mat &dst2)
{
  ...
  cv::Mat res1, res2; //本当はdst1, dst2だったもの

  /* 2画像を比較して2辺とも小さい場合はsrc2に合わせて補正できるように */
    if(src1.cols <= src2.cols && src1.rows <= src2.rows){
      res1 = src2.clone();
      res2 = src1.clone();
      if(DEBUG) std::cout << "src2に合わせる" << std::endl;
    }else{
      res1 = src1.clone();
      res2 = src2.clone();
      if(DEBUG) std::cout << "src1に合わせる" << std::endl;
    }
  ...
  akaze->detectAndCompute(res1, cv::noArray(), key1, des1);
  if(DEBUG) std::cout << "2" << std::endl;
  akaze->detectAndCompute(res2, cv::noArray(), key2, des2);
  /* ORB */
  // cv::Ptr<cv::ORB> orb = cv::ORB::create();
  // orb->detectAndCompute(res1, cv::noArray(), key1, des1);
  // orb->detectAndCompute(res2, cv::noArray(), key2, des2);

  //std::cout << des1 << std::endl;
  if(DEBUG) std::cout << "特徴点検出" << std::endl;
  ...
  if(DEBUG) std::cout << "特徴点マッチングアルゴリズム" << std::endl;

  /* 類似度計算(距離による実装、0に近い値ほど画像が類似) */
  float sim = 0;
  const float THRESHOLD = 400; // 類似度の閾値(仮)
  ...
  /* 画像の類似度が低すぎる場合は終了 */
  if( sim > THRESHOLD || std::isnan(sim) == 1){
  ...
  /* src1を変形 */
  cv::warpPerspective(res1, res1, H, res2.size());
